...
### TLA+
- **Temporal Logic of Actions (TLA)**: язык, основанный на теории множеств, операциях над ними и временной логике.
- **Основные элементы**: состояния, действия `Next`, инварианты.
- **Последовательность состояний**: нет явного параллелизма, процессы моделируются через чередование действий.
- **Абстракции**: поддерживает высокоуровневые абстракции (множества, кортежи, функции).

### PRISM
- **Вероятностные системы**: язык ориентирован на вероятностные и временные системы (DTMC, CTMC, MDP).
- **Основные элементы**: модули, переменные, команды `[action] guard → rate: update`.
- **Переходы**: поддерживает вероятностные переходы `0.5:x'=1 + 0.5:x'=0`.
...
### TLA+
- **TLC Model checking**: использует LTL-подобную логику TLA для описания поведения распределенных систем во времени.
- **Акцент на качество модели**: при работе сознательно жертвует оптимизацией (как языка, так и среды разработки) в угоду более полному перебору состояний системы и предоставлению информации о ее поведении.
- **Отсутсвие привычных свойств**: не поддерживаются обычные спецификации формата LTL и CTL, все спецификации задаются на основе вероятностей.

## PRISM
- **Probabilistic Model Checking**: использует вероятностные алгоритмы для проверки вероятностных моделей, например, цепей Маркова или различных автоматов.
- **Анализ вероятностынх свойств**: анализирует поведение системы с течением времени, позволяет задать вероятность переходов между различными состояниями (значениями переменных).
- **Спецификация на основании путей исполнения**: не поддерживает спецификации классических LTL и CTL, все спецификации задаются на основе вероятности прохождения той или иной трассы исполнения.
...
- **Симуляция**: поддержка симуляции моделей для отладки и анализа.
...
### TLA+
- **TLC**: базируется на линейной темпоральной логике, принимает в качестве атомарных формул не только состояния, но и действия.
- **Инварианты**: поддерживает задание инвариантных свойств.
- **Трекинг трасс исполнения и покрытия**: после запуска позволяет отследить трассы исполнения и значения параметров на каждом конкретном шаге.

### PRISM
- **PCTL**: использует расширение вычислительной темпоральной логики, которое позволяет оперировать вероятностью выполнения определенных свойств.
- **Вероятностные свойства**: свойства для верификации задаются с помощью вероятностей наступления события на множестве трасс исполнения.
...
- **TLA+** является переходным этапом от логики систем к логике вероятностей, все еще сохраняя достаточно сложные типы данных и отношения переходов, но вводя вероятностные свойсвта для всех объектов.
- **PRISM** ориентирован целиком и полностью на проверку вероятностных систем и автоматов.
...
Помимо этих моделей в репозитории расположены модели для PRISM (`model`) и для TLA+ (`model.tla`). Для работы с этими моделями рекомендуется использовать соотвествующие IDE — PRISM 4.x.x и TLA+ Toolbox (Java 8).
